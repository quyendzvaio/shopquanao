...
final class ProactiveStylingStateStore
{
    ...
    public function get(int $userId, string $sessionId): array
    {
        ...
        foreach (['pending_product_id', 'pending_variant_id', 'remaining_user_turns', 'suggested_anchor_product_id'] as $key) {
    ...
    public function put(int $userId, string $sessionId, array $state): void
    {
        $sql = 'INSERT INTO proactive_styling_state
            (user_id, session_id, pending_product_id, pending_variant_id, remaining_user_turns, source_event_id, eligible, suggested_anchor_product_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ON DUPLICATE KEY UPDATE pending_product_id=VALUES(pending_product_id), pending_variant_id=VALUES(pending_variant_id),
              ...
              suggested_anchor_product_id=VALUES(suggested_anchor_product_id), updated_at=CURRENT_TIMESTAMP';
